...
const CreatePost = () => {
    ...
    const handleSubmit = async () => {
        console.log(blog);
        if(!blog.title || !blog.content) alert("Please fill all required fields")
    ...
    return (
        <Card sx={{ p: 4, py: 5, maxWidth: "670px", margin: "50px auto", display: "flex", flexDirection: "column", gap: 4, borderRadius: "15px" }} elevation={10}>
            ...
            <TextField multiline required id="outlined-basic" label="title" variant="outlined" name='title' onChange={handleChange} value={blog.title} />
            <TextField id="outlined-basic" label="imageURL" variant="outlined" name='image' onChange={handleChange} value={blog.image} />
            <TextField multiline required id="outlined-basic" label="content" variant="outlined" name='content' rows={7} onChange={handleChange} value={blog.content} />
            <SpeedDial
